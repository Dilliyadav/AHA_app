...
def registration(request):
    ...
    if request.method=='POST' and request.FILES:
        ...
        if ufd.is_valid() and pfd.is_valid():
            MDUO=ufd.save(commit=False)
            password=ufd.cleaned_data['password']
            MDUO.set_password(password)
            MDUO.save()
            MDPO=pfd.save(commit=False)
            MDPO.username=MDUO
            MDPO.save()
            send_mail('Registratioon',"SUCCESSFULLY REGISTERED IN AHA",'vinjamdilli@gmail.com',[MDUO.email],fail_silently=False)
